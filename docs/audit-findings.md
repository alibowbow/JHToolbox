...
### AF-039 — PDF→HWPX "Keep original look" (fidelity) mode + HWPX package backbone
- **Severity:** feature (toward the PDF↔HWPX overhaul; P1 of the spec)
- **Tools/files:** `pdf-to-hwpx`; **new** `lib/hwpx/{units,xml-builders,package-writer,package-validator,pdf-raster-converter}.ts`, `lib/document-model/types.ts`, `scripts/checks/{hwpx-units,hwpx-package}.check.mjs`, `scripts/checks/{resolve.hooks,register-hooks}.mjs`; `lib/processors/hwpx.ts`
- **Observed:** PDF→HWPX only did flat text extraction (fixed Y-buckets), losing layout; HWPX was a minimal single-section package.
- **Implemented:** A modular HWPX backbone. `units` (pt↔HWPUNIT, 1pt = 100). `package-writer` builds a **multi-section** HWPX (one section per page) preserving each page's **size + orientation**, embedding a full-page image per page in `BinData` (`hp:pic` + `binaryItemIDRef`), reusing the shipping OWPML namespaces; mimetype is the first entry and STORE-uncompressed. `package-validator` checks mimetype, required parts, well-formed XML, spine/section/BinData reference integrity, unique ids, and page sizes. `pdf-raster-converter` (browser) renders each page (pdf.js) at a chosen DPI honoring rotation → fidelity HWPX. Wired as the **default "Keep original look"** mode of `pdf-to-hwpx`; the prior text path is kept as the **"Editable text" (experimental)** mode. HWPX→PDF input now runs a zip-safety guard (traversal/bomb/entry-count). Registry options: mode / dpi (144/200/300) / imageFormat / jpegQuality (en + ko).
- **Regression test:** `hwpx-package.check.mjs` — **16 cases** (builds a 2-page HWPX with different sizes incl. landscape; mimetype first+STORE via raw-ZIP bytes; validator ok; section/BinData/page-size integrity; XML escaping; empty-doc rejection). `hwpx-units.check.mjs` — **8 cases**. A Node resolve hook lets these import the real TS modules.
- **Status:** ✅ structurally verified (typecheck, lint, build 128 pages, 8+16 unit assertions; the package is a valid, internally-consistent ZIP/OWPML). ⚠️ **NOT verified:** that the generated HWPX renders in Hancom Office, and the browser page-rendering path (canvas), because neither Hancom nor a real browser is available here — these need confirmation in a browser / Hancom (or via rhwptopdf once integrated).
- **Deferred:** **P0 HWPX→PDF via rhwptopdf WASM** — the user chose "decide later", and downloading/vendoring the third-party WASM is a safety-gated supply-chain action awaiting explicit authorization; the existing HTML-based HWPX→PDF is unchanged. **P2 editable-mode** full layout analysis (line clustering / reading order / table detection) — the existing text path remains as the experimental mode.
