...
export function BottomNav() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-base-subtle/90 backdrop-blur md:hidden">
      ...
      >
        <nav aria-label={messages.shell.navigate} className="mx-auto flex min-w-max gap-2 rounded-[1.4rem] border border-border bg-base-elevated/75 p-2 shadow-card">
          {items.map(({ href, key, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href} className="shrink-0 rounded-xl2">
                <motion.div
                  tabIndex={-1}
                  whileTap={{ scale: 0.95 }}
