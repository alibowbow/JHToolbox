...
export function CategoryPageContent({
  ...
  return (
    <ToolPageLayout
      ...
      iconBg={style.iconBg}
      meta={<span className="badge border border-border bg-base-elevated text-ink-muted">{formatToolCount(locale, items.length)}</span>}
    >
      {sections ? (
          ...
          {sections.map((section) => (
            <section key={section.id} className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs tabular-nums text-ink-faint">{section.tools.length}</span>
